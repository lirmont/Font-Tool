...
namespace ColorControl
{
	public partial class ColorControl : Form
	{
		...
		private void paletteMode_Draw(bool colorPicking = false)
		{
			...
			GL.PushAttrib(AttribMask.CurrentBit);
			{
				GL.PushMatrix();
				{
					...
					{
						GL.Translate(PaletteGutter.X + ProjectionBoxScreenCoordinates.Left + ProjectionBoxScreenCoordinates.Width / 2.0, PaletteGutter.Y + ProjectionBoxScreenCoordinates.Bottom - ProjectionBoxScreenCoordinates.Height / 2.0, PaletteGutter.Z);
						GL.Scale(PaletteZoomFactor, PaletteZoomFactor, 1.0);
						// TODO: Layout.
		...
		{
			int drawBuffer = (int)DrawBufferMode.Back;
			GL.GetInteger(GetPName.DrawBuffer, out drawBuffer);
			//
			GL.DrawBuffer(DrawBufferMode.Aux0);
			...
			GL.ReadPixels((int)point.x, (int)(simpleOpenGlControl.Location.Y + viewport[3] - point.y), 1, 1, PixelFormat.Rgba, PixelType.UnsignedByte, pixel);
			Color colorAtPoint = Color.FromArgb(pixel[3], pixel[0], pixel[1], pixel[2]);
			//
			GL.DrawBuffer((DrawBufferMode)drawBuffer);
			GL.ReadBuffer((ReadBufferMode)drawBuffer);
			//
