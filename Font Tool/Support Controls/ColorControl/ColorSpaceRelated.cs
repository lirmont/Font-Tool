...
namespace ColorControl {
	public partial class ColorControl : Form
	{
		...
		{
			get { return Math.Round(hueDegree, MidpointRounding.AwayFromZero); }
			set
			{
				hueDegree = value;
		...
		{
			get { return Math.Round(saturationZeroToOne * 100, MidpointRounding.AwayFromZero) / 100.0; }
			set
			{
		...
		{
			get { return Math.Round(valueZeroToOne * 100, MidpointRounding.AwayFromZero) / 100.0; }
			set
			{
		...
		private void updateColorSpaceComponents(Type ignore = null)
		{
			if (!ignoreTextChanges)
			{
				...
				ignoreTextChanges = true;
				{
					...
					// Set the lightness percent for the value circle.
					lightnessPercent = (1.0 - Lab.L / 100.0);
				}
				ignoreTextChanges = false;
				colorDisplayPanel.BackColor = Color;
			}
